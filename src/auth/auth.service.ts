...
import { PROTECTED_ROLES } from '../interfaces/users/roles.type';
...
export class AuthService {
    ...
    if (existing) {
      if (!existing.is_email_verified) {
        throw new BadRequestException({
          message: 'Account exists but email is not verified. Check your inbox.',
          success: false,
        });
      }
      throw new BadRequestException({
  ...
  async verifyEmail(dto: VerifyEmailDto) {
    ...
    if (!user) {
      throw new BadRequestException({ message: 'User not found.', success: false });
    }
    if (user.is_email_verified) {
      throw new BadRequestException({ message: 'Email is already verified.', success: false });
    }
    ...
    if (!tokenRecord || tokenRecord.token !== dto.token) {
      throw new BadRequestException({ message: 'Invalid verification code.', success: false });
    }
    if (tokenRecord.expires_at && new Date() > tokenRecord.expires_at) {
      throw new BadRequestException({ message: 'Verification code has expired.', success: false });
    }
  ...
  async login(dto: LoginDto) {
    const user = await this.findByEmail(dto.email);
    if (!user) {
      throw new BadRequestException({ message: 'Invalid credentials.', success: false });
    }
    if (!user.is_email_verified) {
      throw new BadRequestException({ message: 'Please verify your email before logging in.', success: false });
    }
    if (!user.is_active) {
      throw new ForbiddenException({ message: 'Your account has been deactivated.', success: false });
    }
    if (!user.password) {
      throw new BadRequestException({ message: 'This account uses invite-based login.', success: false });
    }
  ...
  async resetPassword(dto: ResetPasswordDto) {
    const user = await this.findByEmail(dto.email);
    if (!user) {
      throw new BadRequestException({ message: 'Invalid request.', success: false });
    }
    ...
    if (!resetRecord || resetRecord.token !== dto.token) {
      throw new BadRequestException({ message: 'Invalid reset code.', success: false });
    }
    if (resetRecord.expires_at && new Date() > resetRecord.expires_at) {
      throw new BadRequestException({ message: 'Reset code has expired.', success: false });
    }
  ...
  async refreshTokens(refreshToken: string) {
    try {
      ...
      if (!user || user.refresh_token !== refreshToken) {
        throw new BadRequestException({ message: 'Invalid refresh token.', success: false });
      }
      ...
      throw new BadRequestException({ message: 'Invalid or expired refresh token.', success: false });
    }
  }
  ...
  async completeInvite(dto: CompleteInviteDto) {
    ...
    if (!invite) {
      throw new BadRequestException({ message: 'Invalid or already used invitation.', success: false });
    }
    if (new Date() > invite.expires_at) {
      ...
      throw new BadRequestException({ message: 'This invitation has expired.', success: false });
    }

    const existing = await this.findByEmail(invite.email);
    if (existing) {
      throw new BadRequestException({ message: 'This email is already registered.', success: false });
    }
  ...
  async getMe(userId: number) {
    ...
    if (!user) {
      throw new NotFoundException({ message: 'User not found.', success: false });
    }
